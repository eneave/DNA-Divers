...
p3 <- p3[c(2:54)]
## Phase 4 - all samples with tele02 marker
p4 <- read.csv("C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/motu98_final_p4.csv")
p4 <- p4[c(2:45)]
...
library(ggplot2)
#####
## 1st Sequencing Run - All UK Samples - Tele02 primer 
#####
...
View(p2e_aq)
p2e_aq_v1 <- p2e_aq %>% select(c(id, sample.10Ae60BLUE_MPEtB:sample.9He60BLUE_MPEtA_)) %>%
...
p2e_aq_v3 <- merge(p2e_aq_v2,p2e_aq[c(1:10)], by="id") # add taxonomy back 
# list of seq_ids
p2e_aq_names <- as.data.frame(colnames(p2e_aq_v2[-c(1)])) # 28 samples
colnames(p2e_aq_names)[1] <- "seq_id" # can use list to compare to sample data to make sure they're in the same order
# read in metadata
p2e_aq_meta <- read.csv("C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/metadata/p2e_aq_meta.csv")
...
sample_info_tab <- merge(p2e_aq_names,p2e_aq_meta, by="seq_id", all.x = T) %>% ##merge to make sure we have metadata for all of your samples
  column_to_rownames(var = "seq_id")
# partition MOTU table to only include taxa
tax_tab <- p2e_aq_v3 %>% select(c(id, final_class, final_order, final_genus, final_name)) %>% 
            column_to_rownames(var = "id")
# create phyloseq object
OTU = otu_table(as.matrix(count_tab), taxa_are_rows = TRUE)
TAX = tax_table(as.matrix(tax_tab))
...
write.csv(p2e_aq_prev, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/decontam/p2e_aq_decontam.csv")
## save a list of the identified contaminants
write.csv(contamdf.prev05T, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/decontam/p2e_aq_contamdf_prev05T.csv")
## remove generic named data, since same names will be used for different MOTU tables
rm(count_tab,sample_info_tab,tax_tab,OTU,TAX,SAM,dataset,df) 
rm(contamdf.prev,contamdf.prev05,contamdf.prev05T,contamdf.prevT)
#####
...
View(p2t_aq)
count_tab <- p2t_aq %>% select(c(id, sample.5Et10BLUE_MPEtA_:sample.8Ft_EBJun_13extblank)) %>%
  column_to_rownames(var = "id") # make MOTU ids row names
p2t_aq_v1 <- p2t_aq %>% select(c(id, sample.5Et10BLUE_MPEtA_:sample.8Ft_EBJun_13extblank))# have a version of count_tab with a MOTU ID column
p2t_aq_v2 <- count_tab %>% rownames_to_column(var = "id")
p2t_aq_v3 <- merge(p2t_aq_v2,p2t_aq[c(1:10)], by="id") # add taxonomy back; no need since all taxa have been kept
# list of seq_ids
p2t_aq_names <- as.data.frame(colnames(p2t_aq_v1[-c(1)])) 
colnames(p2t_aq_names)[1] <- "seq_id" # can use list to compare to sample data to make sure they're in the same order
# read in metadata
p2t_aq_meta <- read.csv("C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/metadata/p2t_aq_meta.csv")
...
sample_info_tab <- merge(p2t_aq_names,p2t_aq_meta, by="seq_id", all.x = T) %>% ##merge to make sure we have metadata for all of your samples
  column_to_rownames(var = "seq_id")
# partition MOTU table to only include taxa
tax_tab <- p2t_aq %>% select(c(id, final_class, final_order, final_genus, final_name)) %>% 
          column_to_rownames(var = "id")
# create phyloseq object
OTU = otu_table(as.matrix(count_tab), taxa_are_rows = TRUE)
TAX = tax_table(as.matrix(tax_tab))
...
## use 0.5 prevalence threshold
p2t_aq_prev <- p2t_aq[!(p2t_aq$id %in% contamdf.prev05T$id),] 
write.csv(p2t_aq_prev, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/decontam/p2t_aq_decontam.csv")
## save a list of the identified contaminants
write.csv(contamdf.prev05T, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/decontam/p2t_aq_contamdf_prev05T.csv")
## remove generic named data, since same names will be used for different MOTU tables
## remove generic named data, since same names will be used for different MOTU tables
rm(count_tab,sample_info_tab,tax_tab,OTU,TAX,SAM,dataset,df)
rm(contamdf.prev,contamdf.prev05,contamdf.prev05T,contamdf.prevT)
...
# partition MOTU table to only include counts
View(p2t_uk)
p2t_uk_v1 <- p2t_uk %>% select(c(id, sample.4AtLIV_eDNA_FBblank:sample.8HtLIV_FBEtblank)) %>%
  column_to_rownames(var = "id") # make MOTU ids row names
...
p2t_uk_v3 <- merge(p2t_uk_v2,p2t_uk[c(1:10)], by="id") # add taxonomy back 
# list of seq_ids
...
# read in metadata
p2t_uk_meta <- read.csv("C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/metadata/p2t_uk_meta.csv")
...
sample_info_tab <- merge(p2t_uk_names,p2t_uk_meta, by="seq_id", all.x = T) %>% ##merge to make sure we have metadata for all of your samples
  column_to_rownames(var = "seq_id")
# partition MOTU table to only include taxa
tax_tab <- p2t_uk_v3 %>% select(c(id, final_class, final_order, final_genus, final_name)) %>% 
  column_to_rownames(var = "id")
# create phyloseq object
OTU = otu_table(as.matrix(count_tab), taxa_are_rows = TRUE)
TAX = tax_table(as.matrix(tax_tab))
...
write.csv(p2t_uk_prev, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/decontam/p2t_uk_decontam.csv")
## save a list of the identified contaminants
write.csv(contamdf.prev05T, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/decontam/p2t_uk_contamdf_prev05T.csv")
## remove generic named data, since same names will be used for different MOTU tables
rm(count_tab,sample_info_tab,tax_tab,OTU,TAX,SAM,dataset,df) 
rm(contamdf.prev,contamdf.prev05,contamdf.prev05T,contamdf.prevT)
...
# partition MOTU table to only include counts
View(p2e_uk)
p2e_uk_v1 <- p2e_uk %>% select(c(id, sample.11Be_EBMay_13extblank:sample.8BeORK_MPbC_lisa)) %>%
  column_to_rownames(var = "id") # make MOTU ids row names
...
p2e_uk_v3 <- merge(p2e_uk_v2,p2e_uk[c(1:10)], by="id") # add taxonomy back 
# list of seq_ids
...
# read in metadata
p2e_uk_meta <- read.csv("C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/metadata/p2e_uk_meta.csv") 
...
sample_info_tab <- merge(p2e_uk_names,p2e_uk_meta, by="seq_id", all.x = T) %>% ##merge to make sure we have metadata for all of your samples
  column_to_rownames(var = "seq_id")
# partition MOTU table to only include taxa
tax_tab <- p2e_uk_v3 %>% select(c(id, final_class, final_order, final_genus, final_name)) %>% 
  column_to_rownames(var = "id")
# create phyloseq object
OTU = otu_table(as.matrix(count_tab), taxa_are_rows = TRUE)
TAX = tax_table(as.matrix(tax_tab))
...
write.csv(p2e_uk_prev, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/decontam/p2e_uk_decontam.csv")
## save a list of the identified contaminants
write.csv(contamdf.prev05T, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/decontam/p2e_uk_contamdf_prev05T.csv")
## remove generic named data, since same names will be used for different MOTU tables
rm(count_tab,sample_info_tab,tax_tab,OTU,TAX,SAM,dataset,df) 
rm(contamdf.prev,contamdf.prev05,contamdf.prev05T,contamdf.prevT)
...
#####
## 3rd Sequencing Run - All Samples - Elas02 primer
#####
# Convert to phyloseq objects 
# partition MOTU table to only include counts
