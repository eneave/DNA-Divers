...
#eco_v1 <- eco %>% 
#  unnest(id) %>%
#  separate(id, c("id", "size", "count", "id_status", "family", "species_name", "best_match",
#                 "taxid_by_db", "rank_by_db", "scientific name",
#                 "match_count", "rank", "taxid", "species",
#                 "order_name", "best_identity", "scientific_name_by_db",
#                 "spcies_list", "genus_name", "family_name", "genus", "order"), sep = "\\;",
...
# Remove columns with samples from a different project
abund <- subset(abund, select = -c(sample.9AtFL_MKO591_sponge, sample.9BtFL_MKO1174_sponge,
                                   sample.9CtFL_MKO1184_sponge, sample.9DtFL_MKO1172_sponge,
                                   sample.9EtFL_GLOVEe2_sponge, sample.9FtFL_VASEe3_sponge,
                                   sample.9HtFL_BROWNe2_sponge))
# Recalculate total reads with samples removed
abund$total_reads <- rowSums(abund[c(2:40)])
# Join the dataframes to make raw motu table 
# put all data frames into list
df_list <- list(eco_clean, tax_clean, bn_clean, abund)
# merge all data frames in list & emove motus with total reads = 0
motu_all <- df_list %>% reduce(full_join, by="id") %>%
  filter(total_reads > 0)

### Manually correct synonyms in ecotag data
conditions <- c("Plectorhinchus orientalis")
replacement_values <- c("Plectorhinchus vittatus")

motu_all$scientific_name <- replace(motu_all$scientific_name, motu_all$scientific_name %in% conditions, replacement_values)
...
# keep assignments assigned with ecotag over 70%
...
write.csv(motu_all_ident, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/motu70_all_p2t.csv") # sequence run 2, tele02 library
...
write.csv(motu98, "C:/Users/beseneav/OneDrive - Liverpool John Moores University/PhD/chapter3_dnadivers/DNA-Divers/data/motu98_final_p2t.csv") # sequence run 2, tele02 library
